...
$(document).ready(function () {
    ...
    $('#create_item_form').find('[name=institution_role], [name=profile]').on('change', function () {
        var _value = $(this).val();
        var inst = current.institutioniD;
        ...
        if ($('[name=institution_role]').val() == 4 || $('[name=institution_role]').val() == 3) {
            $('.student-teacher').removeClass('d-none')
        } else { $('.student-teacher').addClass('d-none') }
    });
...
$('#btn_create_item').on('click', function () {
    ...
    $('.dynamic-field').each(function () {
        ...
        if (this.type === 'file' && this.files.length > 0) {
            ...
            formData.append('institution', current.institutioniD);
    ...
    $.ajax({
        url: site + "/api/admin-create-account",
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
